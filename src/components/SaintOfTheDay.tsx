...
const SaintOfTheDay = () => {
  ...
  const saying = saint.sayings[0]?.[language] || "";
  ...
  return (
    <Link to={`/saints/${saint.slug}`}>
      <ContentCard gold copyText={copyText} className="overflow-hidden cursor-pointer hover:border-gold transition-all">
        {/* Header */}
        <div className="flex items-center gap-3 mb-3">
          <Avatar className="h-12 w-12 border-2 border-gold/30">
            <AvatarImage src={saint.image} alt={name} />
